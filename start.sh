...
if [[ -n "${FORGE_TOKENS:-}" ]] && [[ "${FORGE_TOKENS}" != '{"github.com": ""}' ]]; then
    /app/pkg/ps-gen-projects -projects /app/data/site/data/projects.yaml 2>/dev/null > /app/data/site/data/projects_gen.yaml || echo "Warning: Failed to generate projects"
else
...
if [[ -n "${FORGE_TOKENS:-}" ]] && [[ "${FORGE_TOKENS}" != '{"github.com": ""}' ]]; then
    /app/pkg/ps-gen-starred -username "${GITHUB_USERNAME:-rmdes}" -limit 30 2>/dev/null > /app/data/site/data/starred_gen.yaml || echo "Warning: Failed to generate starred repos"
else
